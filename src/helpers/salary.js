...
function calculateSalaryFrom(fullSalary) {
  ...
  const percentDiscountINSS = ((discountINSS / baseINSS) * 100).toFixed(2);
  const percentDiscountIRPF = ((discountIRPF / baseINSS) * 100).toFixed(2);
  const percentNetSalary = ((netSalary / baseINSS) * 100).toFixed(2);
